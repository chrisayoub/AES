...
def decrypt(keysize, keyfile, inputfile, outputfile):
    global rounds
...
def mix_cols_decrypt(matrix):
    for col in range(L):
        ...
        # New values
        matrix[0][col] = \
            mul14[old[0]] ^ mul11[old[1]] ^ mul13[old[2]] ^ mul9[old[3]]

        matrix[1][col] = \
            mul9[old[0]] ^ mul14[old[1]] ^ mul11[old[2]] ^ mul13[old[3]]

        matrix[2][col] = \
            mul13[old[0]] ^ mul9[old[1]] ^ mul14[old[2]] ^ mul11[old[3]]

        matrix[3][col] = \
            mul11[old[0]] ^ mul13[old[1]] ^ mul9[old[2]] ^ mul14[old[3]]
