...
do
    ...
    # Get input file
    dd if=/dev/urandom of=in_rand bs=1 status=none count=$i
    # Get key 128
    dd if=/dev/urandom of=key_128 bs=1 status=none count=16
    # Get key 256
    dd if=/dev/urandom of=key_256 bs=1 status=none count=32
    # Execute encrypt 128
